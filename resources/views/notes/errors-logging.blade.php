...
@section('content')
  ...
  <p>Some exceptions describe HTTP error codes from the server. For example, this may be a "page not found" error (404), an "unauthorized error" (401) or even a developer generated 500 error. In order to generate such a response from anywhere in the application, we can use the abort() helper:</p>

  <pre><code class="language-php">
    abort(404);
  </code></pre>

  <p>The abort() helper will immediately raise an exception which will be rendered by the exception handler.Optionally, we can provide the response text:</p>
  ...
  <p>Laravel makes it easy to display custom error pages for various HTTP codes. For example, to customize the error page for 404 HTTP status codes, we can create a resources/views/errors/404.blade.php file. This file will be served on all 404 errors generated by the application. The views within this directory should be named to match the HTTP status code they correspond to. The HttpException instance raised by the abort() function will be passed to the view as an $exception variable:</p>
